...
namespace Bloxstrap
{
    public class Bootstrapper
    {
        ...
        private void LaunchEnabledExternals()
        {
            const string LOG_IDENT = "Bootstrapper::LaunchEnabledExternals";

            foreach (var option in App.Settings.Prop.ExecutorsExternal)
            {
                App.Logger.WriteLine(LOG_IDENT,
        $"{option.Name}: Enabled={option.Enabled}, Folder='{option.Folder}'");
                if (!option.Enabled)
                    continue;

                if (string.IsNullOrWhiteSpace(option.Folder) || !Directory.Exists(option.Folder))
                {
                    App.Logger.WriteLine(LOG_IDENT, $"{option.Name} is enabled but its folder is missing or unset ({option.Folder})");
                    continue;
                }

                // first .exe found in the directory (top level only)
                string? exe = Directory
                    .EnumerateFiles(option.Folder, "*.exe", SearchOption.TopDirectoryOnly)
                    .OrderBy(f => f, StringComparer.OrdinalIgnoreCase)
                    .FirstOrDefault();

                if (exe is null)
                {
                    App.Logger.WriteLine(LOG_IDENT, $"{option.Name} is enabled but no .exe was found in {option.Folder}");
                    continue;
                }

                // skip if it's already running
                string processName = Path.GetFileNameWithoutExtension(exe);
                if (IsProcessRunning(processName, exe))
                {
                    App.Logger.WriteLine(LOG_IDENT, $"{option.Name} ({processName}) is already running, skipping");
                    continue;
                }

                try
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Launching {option.Name}: {exe}");

                    Process.Start(new ProcessStartInfo
                    {
                        FileName = exe,
                        WorkingDirectory = option.Folder,
                        UseShellExecute = true
                    });
                }
                catch (Exception ex)
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Failed to launch {option.Name}");
                    App.Logger.WriteException(LOG_IDENT, ex);
                }
            }
        }
        private void LaunchEnabledInternals()
        {
            const string LOG_IDENT = "Bootstrapper::LaunchEnabledInternals";

            foreach (var option in App.Settings.Prop.ExecutorsInternal)
            {
                App.Logger.WriteLine(LOG_IDENT,
        $"{option.Name}: Enabled={option.Enabled}, Folder='{option.Folder}'");
                if (!option.Enabled)
        ...
        private static bool IsProcessRunning(string processName, string exePath)
        {
            foreach (var process in Process.GetProcessesByName(processName))
            {
                try
                {
                    // a process with the same name could be a different exe elsewhere on disk;
                    // match the actual file path to be sure it's *this* one
                    if (string.Equals(process.MainModule?.FileName, exePath, StringComparison.OrdinalIgnoreCase))
                        return true;
                }
                catch
                {
                    // MainModule can throw (access denied / exited); fall back to name-only match
                    return true;
                }
                finally
                {
        ...
        private void ClearRobloxStorage()
        {
            const string LOG_IDENT = "Bootstrapper::ClearRobloxStorage";

            string roblox = Path.Combine(Paths.LocalAppData, "Roblox");
            App.Logger.WriteLine(LOG_IDENT, $"Clearing Roblox storage in {roblox}");
        ...
        "LocalStorage"
            ...
            {
                string path = Path.Combine(roblox, name);
                try
                {
                    if (File.Exists(path))
                        File.Delete(path);
                ...
                {
                    // matches the batch's "2>nul" — ignore failures (missing/locked)
                    App.Logger.WriteLine(LOG_IDENT, $"Could not delete file {name}: {ex.Message}");
                }
            }
            ...
            foreach (string name in folders)
            {
                ...
                try
                {
                    if (Directory.Exists(path))
                        Directory.Delete(path, true); // true = recursive, like rmdir /s
                }
                catch (Exception ex)
                {
                    App.Logger.WriteLine(LOG_IDENT, $"Could not delete folder {name}: {ex.Message}");
                }
            }

            App.Logger.WriteLine(LOG_IDENT, "Done clearing Roblox storage");
        }
        private async Task StartRoblox()
        {
            ...
            LaunchEnabledExternals();
            LaunchEnabledInternals();
            if (App.Settings.Prop.Spoofer)
            {
                ClearRobloxStorage();
            }
            if (_launchMode == LaunchMode.Player)
